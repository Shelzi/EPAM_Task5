    ...
    public static Purchase getPurchaseFromFactory(Scanner sc) {
        ...
        switch (sign) {
            case GENERAL_PURCHASE:
                return PurchaseKind.GENERAL_PURCHASE.getPurchase(sc);
            case PRICE_DIS_PURCHASE:
                return PurchaseKind.PRICE_DIS_PURCHASE.getPurchase(sc);
            case PERCENT_DIS_PURCHASE:
                return PurchaseKind.PERCENT_DIS_PURCHASE.getPurchase(sc);
            default:
                throw new EnumConstantNotPresentException(PurchaseKind.class, sign.name());
        }
    }
}